...
app.get('/api/migrate', async (req, res) => {
    try {
        await pool.query(`
            ...
            ADD COLUMN IF NOT EXISTS industry_priority INTEGER
        `);
...
app.post('/api/videos', async (req, res) => {
    const { filename, display_name, url, thumbnail_url, duration_seconds } = req.body;
    try {
        const result = await pool.query(
            `INSERT INTO videos (filename, display_name, url, thumbnail_url, duration_seconds)
             VALUES ($1, $2, $3, $4, $5) RETURNING *`,
            [filename, display_name, url, thumbnail_url, duration_seconds || null]
        );
        res.status(201).json(result.rows[0]);
    } catch (err) {
